...
**At home** (`maps/lab_map.yaml`):
- Generated by `scripts/world_to_map.py` from `worlds/lab_world.sdf`
- Walls are **perfect and straight** — mathematically computed from the SDF geometry
- Contains only what is declared in the `.sdf` file (no furniture, no real obstacles)
- Origin (0, 0) = computed bounding box min corner of all walls — fixed and deterministic
- Every run at home produces the **exact same map**

**At lab** (`~/map.yaml`):
- Generated by **SLAM** (Cartographer) as the real robot drives around the room
- Walls are **slightly jagged** due to LiDAR sensor noise
- Contains **everything the LiDAR sees**: furniture, equipment, people, cables
- Origin (0, 0) = wherever the robot was sitting when SLAM started — changes each run
- Zone coordinates in the code **do not match** this map (they were written for the Gazebo world)

**Consequence:** the hardcoded `WAYPOINTS` and `FARM_ZONES` in the code are valid in
the Gazebo world frame. In the real lab the map frame is anchored to the SLAM start
position, so those same numbers point to completely different physical locations — the
robot will drive to a wrong or unreachable spot and Nav2 will abort the goal.

**Fix before every lab session:** measure real zone positions (step B9) and update the
coordinates.
...
- [ ] **A real SLAM map** `~/map.yaml` of the actual room (B3). The bundled
      `maps/lab_map.yaml` was generated from the Gazebo SDF world — it does **not**
      match the real room and AMCL will fail to localize if you use it on the real robot.
- [ ] **Zone coordinates set to the real room** in `FARM_ZONES` + `WAYPOINTS` (B9),
      ...
      sim world and will be meaningless in the real room — Nav2 will abort every goal.
...
Run SLAM **before** any navigation. The bundled map (`maps/lab_map.yaml`) was
generated from the Gazebo SDF and does **not** match the real room — AMCL cannot
localize with it on the real robot.
...
Drive the robot along **all walls and around all obstacles** in the room so the
LiDAR sees everything. The more complete the scan, the better AMCL will localize later.
...
This creates `~/map.yaml` and `~/map.pgm`. Keep the robot in the **same starting
position** when you later run navigation — zone coordinates (B9) are measured
relative to this SLAM start position.
...
Replace `home_x/home_y` with the robot's real start coordinates if different from (0, 0).
...
If `/start_navigation` returns `aborted`, the most likely causes are:
...
## B9. Adjust zone positions (required before every lab session)

Zone coordinates in the code (`WAYPOINTS` and `FARM_ZONES`) were written for the
Gazebo simulation world. In the real lab they must be updated to match the actual
room, otherwise Nav2 will send the robot to wrong or unreachable positions and
abort every goal.

Place the robot at each zone location in the room and read the coordinates:
```bash
ros2 topic echo /odom --once | grep -A3 "position"
```

Update the **same coordinates** in both files:
- `farm_twin_poc/zone_monitor_node.py` → `FARM_ZONES`
- `farm_twin_poc/navigator_node.py` → `WAYPOINTS`  (Nav2 goals)

Also update `<pose>` in `worlds/lab_world.sdf` if you want the Gazebo twin to
show zone markers at the correct positions.

Rebuild and push:
```bash
git add . && git commit -m "Update zone coordinates for real lab room" && git push
